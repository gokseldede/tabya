...
$(function () {
    ...
    $(window).on('beforeunload', function (e) {
        if (agent.isOnline) {
            var msg = 'Bu sayfayı kapatmadan önce çevrimdışı durumunuzu değiştirin. Aksi takdirde ziyaretçi sizin durumunuzu açık görerek mesaj atabilecektir.';
            e.returnValue = msg;
            return msg;
        }
    });
    ...
    myHub.client.loginResult = function (status, id, name) {
        if (status) {
            ...
            var msg = id == 'config' ?
                'Hiçbir yapılandırma dosyası bulunamadı. Kullanmadan önce LCSK dosyasını yapılandırın lütfen' :
                'Bilgileriniz veritabınında bulunamamıştır.';
    ...
    myHub.client.newVisit = function (page, referrer, city, country, chatWith, connectionId) {
        ...
        $('#current-visits > tbody').prepend(
            ...
            '<td><a href="#" class="engage-visitor" data-id="' + connectionId + '">Otomatik mesaj gönder</a></td></tr>');
    ...
    myHub.client.newChat = function (id) {
        ...
        $('#chat-sessions').prepend(
            ...
            '<p>Yeni mesaj(s) <span class="badge badge-warning">0</span></p></div>' +
            '</div>');
    ...
    myHub.client.visitorSwitchPage = function (lastId, newId, newPage) {
        ...
        var session = [];
        session.push('<strong>system</strong> Ziyeratçi şuan: ' + newPage);
        ...
        if (lastId == getCurrentChatId()) {
            $('#chatmsgs' + lastId).append('<p><strong>system</strong> Ziyaretçi şuan: ' + newPage + '</p>');
...
function commandTriggered(cmd) {
    ...
    if (parts[0] == 'list') {
        ...
        if (modalBody != null) {
            ...
            var body =
                '<p><strong>/list</strong> tüm komutları listele.</p>' +
                '<p><strong>/add command-name text to be sent</strong> yeni komut ekle.</p>' +
                '<p><strong>/edit command-name text to be sent</strong> komut düzenle.</p>' +
                '<p><strong>/del command-name</strong> komut sil.</p>' +
                '<p><strong>/transfer agent-name</strong> konuşmayı başka agent a transfer et.</p>';
        ...
        if (exists == null) {
            ...
            $('#chatmsgs' + getCurrentChatId()).append('<p><strong>system</strong> ' + parts[1] + ' komut eklendi</p>');
        } else {
            $('#chatmsgs' + getCurrentChatId()).append('<p><strong>system</strong> The command <em>' + parts[1] + '</em> düzenleme mevcut durumdadır.</p>');
        }
        ...
        $('#chatmsgs' + getCurrentChatId()).append('<p><strong>system</strong> ' + parts[1] + ' değiştirildi</p>');
    } else if (parts[0] == 'del') {
        var commands = getCommands();
        var newCommands = [];
        ...
        $('#chatmsgs' + getCurrentChatId()).append('<p><strong>system</strong> ' + parts[1] + ' kaldırıldı</p>');
        ...
        if (command != null && chatId != '') {
            ...
            $('#chatmsgs' + chatId).append('<p><strong>system</strong> ' + parts[1] + ' kullanılabilir komutları görüntülemek için tanınan komut, kullanım / liste değildir.</p>');
        }
    }
}
