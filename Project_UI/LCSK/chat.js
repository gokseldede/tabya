...
var LCSKChat = function () {
    ...
    function setDefaults() {
        ...
        options.offlineTitle = 'İletişim!';
        options.onlineTitle = 'Sohbet edin!';

        options.waitingForOperator = 'Teşekkürler, bize sohbet kabul etmek 1 dakika verin ...';
        options.emailSent = 'Epostanız alındı en kısa sürede sizinle iletişime geçilecektir.';
        options.emailFailed = 'E-posta şu anda gönderilemedi.<br /><br />Bunun için üzgünüm.';
        ...
        if (state) {
            ...
            if (!requestChat) {
                $('#chat-box').html(
                    ...
                    '<p>Bir sorunuz mu var? Let\'s chat!</p><p>Aşağıdaki alana sorunuzu ekleyin ve ENTER tuşuna basın.</p></div>' +
            ...
            if (!chatEditing) {
                ...
                $('#chat-box-input').hide();
                $('#chat-box').html(
                    '<p>Email adresiniz</p><input type="text" id="chat-box-email" style="border:1px solid #0354cb;border-radius: 3px;width: 100%;" class="chat-editing" />' +
                    '<p>Mesajınız</p><textarea id="chat-box-cmt" cols="40" rows="7" class="chat-editing" style="border:1px solid #0354cb;border-radius: 3px;"></textarea>' +
                    '<p><input type="button" id="chat-box-send" value="İletişime Geç" />'
                );
            }
        }
    }
    ...
    myHub.client.setChat = function (id, agentName, existing) {
        ...
        if (existing) {
            ...
            $('#chat-box-msg').append('<p>Sohbete devam eden temsilci <strong>' + agentName + '</strong></p>');
        } else {
            $('#chat-box-msg').append('<p>Şimdi sohbet edebilirsiniz. <strong>' + agentName + '</strong></p>');
        }
    };
